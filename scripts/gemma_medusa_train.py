...
Phase 2 of the Gemma Medusa plan: training pipeline for MTP heads.

The goal is to train Medusa heads to predict what the frozen base model would output.
This means the training data should match the base model's output distribution.

Recommended data sources (in order of preference):
1. Self-distillation data (Phase 4) - generated by the base model itself
2. OpenHermes-2.5 (teknium/OpenHermes-2.5) - high quality, diverse, 1M samples
3. UltraChat-200K (HuggingFaceH4/ultrachat_200k) - synthetic conversations

Download OpenHermes 2.5:
    from datasets import load_dataset
    ds = load_dataset("teknium/OpenHermes-2.5", split="train")
    ds.to_json("data/openhermes.json")

Example runs:
    # Train on OpenHermes 2.5
    uv run python -m scripts.gemma_medusa_train \
        --data-path data/openhermes.json \
        --output-dir checkpoints/gemma_medusa
    ...
    uv run python -m scripts.gemma_medusa_train \
        ...
        --batch-size 4 \
        --data-path data/openhermes.json \
        --output-dir checkpoints/gemma_medusa_lora
    ...
    uv run python -m scripts.gemma_medusa_train \
        --data-path data/openhermes.json \
        --resume-from checkpoints/gemma_medusa/step_1000
"""
...
if __name__ == "__main__":
    ...
    parser.add_argument("--head-type", type=str, default="lora",
                        choices=["lora", "delta", "full"],
                        help="Type of Medusa head architecture")
    ...
    parser.add_argument("--lora-rank", type=int, default=64,
                        help="LoRA rank (only used if head-type=lora)")
    ...
    parser.add_argument("--no-freeze-base", dest="freeze_base", action="store_false",
                        help="Train base model parameters too")
    ...
    parser.add_argument("--medusa-loss-weight", type=float, default=1.0,
                        help="Weight for Medusa head losses")
    ...
    print0(f"Medusa config: {args.medusa_num_heads} heads, {args.medusa_num_layers} layers, type={args.head_type}")

    # Note: Currently only LoRA heads are fully implemented in model.py
    # Delta and Full heads would require extending GemmaMedusaModel
    if args.head_type != "lora":
        print0(f"WARNING: head_type={args.head_type} not yet fully implemented, using lora")
    ...
    examples_per_step = args.batch_size * ddp_world_size
    assert args.target_batch_size % examples_per_step == 0, \
        f"Target batch size {args.target_batch_size} must be divisible by examples per step {examples_per_step}"
    grad_accum_steps = args.target_batch_size // examples_per_step
    print0(f"Batch size: {args.batch_size} x {ddp_world_size} GPUs = {examples_per_step} examples/step")
    print0(f"Gradient accumulation steps: {grad_accum_steps}")
    print0(f"Effective batch size: {args.target_batch_size}")

    if args.num_iterations > 0:
        num_iterations = args.num_iterations
    else:
        # Calculate from epochs
        iterations_per_epoch = len(train_data) // args.target_batch_size
    ...
    train_loader = medusa_data_generator(
        train_data, tokenizer, args.batch_size, args.max_seq_len, device,
    ...
    def build_val_loader():
        ...
        return medusa_data_generator(
            val_data, tokenizer, args.batch_size, args.max_seq_len, device,
            ddp_rank=ddp_rank, ddp_world_size=ddp_world_size
        )
