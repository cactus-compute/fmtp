"""
Token Type Analysis for Medusa Head Accuracy (CUDA/PyTorch version).
...
Usage:
    python -m scripts.token_type_analysis_cuda \
...
def load_medusa_model(checkpoint_path: str, device: torch.device, dtype: torch.dtype = torch.bfloat16):
    ...
    # Get model config
    model_name = config.get('base_model', 'google/gemma-3-270m-it')
    ...
    print(f"  Heads: {medusa_num_heads}, Layers: {medusa_num_layers}, LoRA rank: {lora_rank}")
...
def generate_with_logging(
    ...
    """
    Generate tokens using simple 2-token speculation while logging each attempt.

    This implements the 1h1t (1 head, 1 token) speculation pattern:
    1. Get main model's next token (token1)
    2. Get medusa head's prediction for the token after that (token2)
    3. Forward both tokens through the model
    4. Check if main model agrees with token2 when given [prompt + token1]
    5. Accept 1 or 2 tokens based on verification
    ...
    # Helper for forward pass with autocast
    ...
    def run_forward(tokens: List[int], return_medusa: bool = True, last_only: bool = True):
        ...
        if is_cuda:
            with torch.amp.autocast('cuda', dtype=dtype):
                return model.forward(input_tensor, return_medusa=return_medusa, last_only=last_only)
        else:
            return model.forward(input_tensor, return_medusa=return_medusa, last_only=last_only)

    # Initial forward pass
    main_logits, medusa_logits = run_forward(output_tokens, return_medusa=True, last_only=True)
    # main_logits: (B, 1, vocab), medusa_logits: (num_heads, B, 1, vocab)

    while num_generated < max_new_tokens:
        # Get base token prediction (greedy)
        token1 = int(torch.argmax(main_logits[0, 0]).item())

        # Check stop token
        if token1 == eos_token_id:
            break

        # Get medusa head prediction (first head, greedy)
        # medusa_logits shape: (num_heads, B, seq_len, vocab)
        token2 = int(torch.argmax(medusa_logits[0, 0, 0]).item())

        # Forward both tokens to verify
        verify_input = output_tokens + [token1, token2]
        verify_logits, new_medusa = run_forward(verify_input, return_medusa=True, last_only=False)
        # verify_logits: (B, seq_len, vocab)

        # Get what the base model predicts after seeing [output_tokens + token1]
        # Position -2 gives us the prediction at the position of token1's output
        # (what the model predicts should come after token1)
        # Actually, we need the prediction for position len(output_tokens) in verify_logits
        # Since input is output_tokens + [token1, token2], and last_only=False,
        # verify_logits has predictions for all positions
        # Position -2 predicts token2's position (after token1)
        pos_after_token1 = len(output_tokens)  # Index in the verify_input where token1 is
        verified_token2 = int(torch.argmax(verify_logits[0, pos_after_token1]).item())

        # Decode tokens for classification
        token1_str = tokenizer.decode([token1])
        token2_str = tokenizer.decode([token2])
        verified_str = tokenizer.decode([verified_token2])
        ...
        if matched:
            # Check if token2 is stop token
            if token2 == eos_token_id:
            ...
            # Use logits from the last position for next iteration
            main_logits = verify_logits[:, -1:, :]
            medusa_logits = new_medusa[:, :, -1:, :]
        else:
            ...
            # Check if verified_token2 is stop token
            if verified_token2 == eos_token_id:
                break

            # Re-run forward with just token1 added to get clean state
            main_logits, medusa_logits = run_forward(output_tokens, return_medusa=True, last_only=True)
...
def main():
    parser = argparse.ArgumentParser(description="Token type analysis for Medusa speculation (CUDA)")
    ...
    model, tokenizer = load_medusa_model(checkpoint_path, device)
    ...
    for i, sample in enumerate(samples):
        ...
        try:
            ...
            output_tokens, records = generate_with_logging(
                model, tokenizer, input_ids, max_new_tokens=args.max_tokens, device=device
            )
